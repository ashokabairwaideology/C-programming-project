...
Begin VB.Form frmReportCashIn 
   ...
   End
End
Attribute VB_Name = "frmReportCashIn"
...
With db_CashIN.Recordset
    ...
    Set l_results = lvw_recordsfound.ListItems.Add(X, , !OR)
                    l_results.SubItems(1) = !Date
                    l_results.SubItems(2) = !givenby
                    l_results.SubItems(3) = !receivedby
                    l_results.SubItems(4) = !amounT
                    l_results.SubItems(5) = !Remarks
    .MoveNext
    Next
End With
hell:
...
StartStg = DT_Start.Value
 Endstg = DT_End.Value
If DataEnvironment1.rsCommand_CashIN.State = adStateClosed Then DataEnvironment1.rsCommand_CashIN.Open
    DataEnvironment1.rsCommand_CashIN.Requery
    DataEnvironment1.rsCommand_CashIN.Requery
    DataEnvironment1.Commands(4).CommandText = "select * from tblcashin WHERE (((tblcashin.Date) Between # " + StartStg + " # And # " + Endstg + " #)) "
    DataEnvironment1.Commands(4).Execute
    DataEnvironment1.rsCommand_CashIN.Requery
    DataEnvironment1.rsCommand_CashIN.Requery
    Load Rpt_CashIN
    Rpt_CashIN.Sections(5).Controls("label22").Caption = Format(Total_sales, "###,###,###.00")
    Rpt_CashIN.Show vbModal
If DataEnvironment1.rsCommand_CashIN.State = adStateOpen Then DataEnvironment1.rsCommand_CashIN.Close
