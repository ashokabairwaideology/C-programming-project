...
<%
	...
	out.println("<h3>"+user+", Congrats! You passed in Level3 </h3>");
%>
...
try
{
	...
	ResultSet rs1=st.executeQuery("select count(*) from questions where testlevel='level4'");
	while(rs1.next())
...
<%
	...
	{
%>
	<b><u><i>	level4 Of level<%=levelcount%></i></u></b>
	<form action="OnlineTest4.jsp">
	<table border=1>
	<th>Q.No.</th><th>Question</th><th>Option1</th><th>Option2</th>
	<th>Option3</th><th>Option4</th><th>Answer</th>
<%
	rs2=st.executeQuery("select question,option1,option2,option3,option4,answer from questions where testlevel='level4'");	
	int k=1;
	while(rs2.next())
	{
%>
