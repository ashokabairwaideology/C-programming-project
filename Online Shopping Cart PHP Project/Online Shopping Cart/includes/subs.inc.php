...
?>
<?php
	function mymail($to,$subject,$message,$headers,$smtp_server,$smtp_port,$smtp_user,$smtp_pass,$charset)
	{
      ...
      $mail->auth($smtp_user, $smtp_pass, AUTH_DETECT, SSL_TRUE, 465);
      $mail->from($to, $to);
      $mail->port(25);
	  $header['Reply-To'] = $to;
	  $header['X-Whatever'] = 'Contact form';
    ...
    Function PutWindow($title,$message,$picture,$width) {
             ...
             echo "<td><img src=\"".$gfx_dir."/".$picture."\" alt=\"".$picture."\"></td>";
