...
<?php
      if (!empty($_POST['shipping'])) { $shipping=$_POST['shipping']; }
      if (!empty($_POST['payment'])) { $payment=$_POST['payment']; }
...
<?php
   ...
   if (CountCart() == 0) {
     ...
     $query = "INSERT INTO `order` (`DATE`,`STATUS`,`SHIPPING`,`PAYMENT`,`CUSTOMERID`,`TOPAY`,`WEBID`,`NOTES`) VALUES ('".Date($date_format)."','1',".$shipping.",".$payment.",'".$customerid."','1','n/a','".$notes."')";
     $sql = mysql_query($query) or die(mysql_error());
     ...
     // first the shipping
     $query = "SELECT * FROM `shipping` WHERE `id` = " . $shipping;
     $sql = mysql_query($query) or die(mysql_error());
     ...
     // now the payment
     $query = "SELECT * FROM `payment` WHERE `id` = " . $payment;
     $sql = mysql_query($query) or die(mysql_error());
     ...
     if (mymail($to, $subject, $full_message, $email_header, $smtp_server, $smtp_port, $smtp_user, $smtp_pass, $charset)) {
        echo "<p>" . $txt['checkout11'] ."</p>";
     }
     else { echo "<p>" . $txt['checkout12'] . "</p>"; }
     
     mymail($sales_mail, $subject, $full_message, $email_header, $smtp_server, $smtp_port, $smtp_user, $smtp_pass, $charset); // no error checking here, because there is no use to report this to the customer
