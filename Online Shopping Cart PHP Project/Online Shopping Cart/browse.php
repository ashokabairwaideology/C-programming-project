  ...
  <?php
 	...
 	if ($action == "list") {
	 	if ($stock_enabled == 1 && IsAdmin() == false) { // filter out products with stock lower than 1
		 	$query = "SELECT * FROM `product` WHERE `STOCK` > 0 AND CATID=".$cat." ORDER BY ".$orderby;
	 	}
	 	else { $query = "SELECT * FROM `product` WHERE CATID=".$cat." ORDER BY ".$orderby; }
    }
    elseif ($action == "shownew") {
	 	if ($stock_enabled == 1 && IsAdmin() == false) { // filter out products with stock lower than 1
            $query = "SELECT * FROM `product` WHERE `STOCK` > 0 AND NEW=1 ORDER BY ".$orderby;
        }
        else { $query = "SELECT * FROM `product` WHERE `NEW` = 1 ORDER BY ".$orderby; }
    }
